...
6. Run `/ranchstats` to see product totals, recorded animal changes, and sales. `/ranches` lists connected ranches. `/employee` looks up one worker's recorded collection, purchases, and sales. `/addemployee` adds a worker before they appear in any recognised webhooks.
...
Animal figures represent changes visible in webhook history. They are **not** assumed to start at five or capped at five, since a ranch can keep a cow slot free for breeding. If animals already existed before the first recorded webhook, the figure will not represent the current live herd. Sale ledger shares are per-event proceeds, not the ranch's current bank balance. Only purchases, sales, and product collection contribute to employee reports; chores are omitted.

The database is stored as `ranch_data.json` in `RAILWAY_VOLUME_MOUNT_PATH` or this folder. Replaying history skips duplicate message IDs. A refresh builds a fresh report from chronological history and replaces the previous figures only after reading succeeds.
